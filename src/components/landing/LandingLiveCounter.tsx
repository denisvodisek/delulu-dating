...
export function LandingLiveCounter() {
  ...
  if (target == null) {
    return (
      <div
        className="h-28 w-full max-w-lg rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest px-6 py-5 md:h-32"
        aria-hidden
      />
    );
  }
  ...
  return (
    <div ref={wrapRef} className="group relative w-full max-w-lg select-none">
      <div
        role="status"
        aria-live="polite"
        className="relative w-full overflow-hidden rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest px-5 py-5 text-left shadow-sm md:px-7 md:py-6"
      >
        <div className="pointer-events-none absolute -top-12 -right-8 h-32 w-32 rounded-full bg-lab-primary/15 blur-2xl" data-float />
        <div className="pointer-events-none absolute -bottom-10 -left-6 h-28 w-28 rounded-full bg-lab-tertiary/10 blur-2xl" data-float />
        <p className="font-lab-mono relative text-[10px] font-semibold tracking-[0.18em] text-lab-on-surface-variant uppercase md:text-xs">
          {t("liveCounterLabel")}
        </p>
        <p className="font-lab-display relative mt-1 text-[2.75rem] leading-none font-extrabold tracking-tight text-lab-on-surface tabular-nums md:text-[3.5rem]">
          <span ref={valueRef} className="inline-block">
            {target.toLocaleString(locale === "zh" ? "zh-HK" : "en-US")}
          </span>
        </p>
        {hint ? (
          <p className="font-lab-mono relative mt-2 text-[10px] text-lab-on-surface-variant md:text-[11px]">
            {hint}
          </p>
        ) : null}
      </div>
