...
export default function ResultClient({
  ...
  useEffect(() => {
    ...
    void import("canvas-confetti").then((mod) => {
      const c = mod.default;
      const studio = ["#c4f312", "#0891b2", "#a1a1aa", "#e4e4e7", "#fafafa"];
      c({ particleCount: 110, spread: 70, origin: { y: 0.58 }, colors: studio });
      window.setTimeout(() => {
        c({
          particleCount: 70,
          angle: 120,
  ...
  return (
    <main className="flex flex-1 flex-col pt-20">
      ...
      <section className="grid min-h-[600px] w-full grid-cols-1 border-lab-outline-variant bg-lab-surface-container-lowest px-4 md:grid-cols-12 md:items-start md:px-16">
        <div className="border-b border-lab-outline-variant py-12 md:col-span-7 md:border-r md:border-b-0 md:py-16 md:pr-12">
          <div className="mb-12">
            <p className="font-lab-mono mb-3 text-xs font-semibold tracking-wide text-lab-on-surface-variant uppercase">
          ...
          <div className="flex flex-col gap-4">
            {debtRows.map((row, idx) => (
              <div
                key={row.key}
                className="grid grid-cols-12 items-center gap-y-3 rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest p-5 shadow-sm md:p-6"
              >
                <div className="font-lab-display col-span-12 text-2xl text-lab-outline-variant md:col-span-1">
          ...
          {tightest ? (
            <div className="mt-10 rounded-xl border border-lab-outline-variant bg-lab-surface p-6 shadow-sm">
          ...
          {(calc.tier === "delulu" || calc.tier === "god") && (
            <div className="mt-8 rounded-xl border-2 border-lab-error/40 bg-lab-error-container p-8 shadow-sm">
        ...
        <div className="flex flex-col gap-8 py-12 md:col-span-5 md:py-16 md:pl-12">
          <div className="sticky top-24 z-20 self-start md:top-28">
            <div className="mb-10 overflow-hidden rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest shadow-md backdrop-blur-sm">
              <div className="border-b border-lab-outline-variant bg-lab-on-surface px-6 py-3">
                <p className="font-lab-mono text-xs font-semibold tracking-wide text-lab-surface-container-lowest uppercase">
                  {t("labClinicalLabel")}
                </p>
              </div>
              <div className="p-8">
                ...
                <div className="mb-6 flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="font-lab-mono rounded-sm border border-lab-outline-variant bg-lab-surface px-3 py-1 text-[10px] font-semibold tracking-wide text-lab-on-surface uppercase"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div
                  className={cn(
                    "relative h-10 w-full overflow-hidden rounded-full border-2 border-lab-outline-variant bg-lab-surface",
                  ...
                  <div className="absolute inset-0 flex items-center justify-end px-4">
                    <span
                      className={cn(
                        ...
                        alarming || sev > 40 ? "text-white" : "text-lab-on-primary",
                      )}
                    >
                      {sev}%
                    </span>
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="font-lab-mono flex w-full flex-col gap-1 rounded-xl border-2 border-lab-on-surface bg-lab-primary py-4 text-xs font-bold tracking-wide text-lab-on-primary uppercase shadow-[3px_3px_0_0_rgb(10,10,11)] transition-all hover:translate-x-px hover:translate-y-px hover:shadow-[2px_2px_0_0_rgb(10,10,11)] disabled:cursor-wait disabled:opacity-70"
            >
      ...
      <section className="w-full border-t border-lab-outline-variant bg-lab-surface px-4 py-16 md:px-16 md:py-24">
        <div className="mx-auto flex max-w-4xl flex-col items-center justify-between gap-8 md:flex-row">
          ...
          <div className="flex w-full flex-col gap-4 sm:flex-row sm:justify-end">
            <button
              ...
              className="font-lab-mono rounded-xl border-2 border-lab-on-surface bg-lab-primary px-8 py-5 text-xs font-bold tracking-widest text-lab-on-primary uppercase shadow-[3px_3px_0_0_rgb(10,10,11)] transition-all hover:translate-x-px hover:translate-y-px hover:shadow-[2px_2px_0_0_rgb(10,10,11)]"
            >
              {t("labRedoCalc")}
            </button>
            <Link
              href="/methodology"
              className="font-lab-mono flex items-center justify-center rounded-xl border-2 border-lab-outline-variant bg-lab-surface-container-lowest px-8 py-5 text-xs font-semibold tracking-widest text-lab-on-surface uppercase transition-all hover:border-lab-on-surface/30 hover:bg-lab-surface"
            >
              {t("labReadMethod")}
            </Link>
          </div>
      ...
      <footer
        ref={footerRef}
        className="relative overflow-hidden border-t border-lab-outline-variant bg-lab-surface-container-lowest px-4 py-14 md:px-16 md:py-16"
      >
        <div className="pointer-events-none absolute -top-16 -left-8 h-44 w-44 rounded-full bg-lab-primary/10 blur-3xl" />
        <div className="pointer-events-none absolute -right-14 bottom-0 h-52 w-52 rounded-full bg-lab-tertiary/10 blur-3xl" />
        <div className="pointer-events-none absolute top-1/2 left-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full bg-lab-outline-variant/25 blur-3xl" />
        ...
        <div className="relative mx-auto max-w-6xl">
          <div data-footer-showcase>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <span className="font-lab-mono rounded-sm border border-lab-outline-variant bg-lab-surface px-3 py-1 text-[10px] font-semibold tracking-wide text-lab-on-surface uppercase">
        ...
        <div className="relative mx-auto mt-10 grid max-w-6xl grid-cols-1 gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-lab-outline-variant bg-lab-surface px-4 py-4 shadow-sm backdrop-blur-sm" data-footer-showcase>
            <p className="font-lab-mono text-[10px] font-semibold tracking-wider text-lab-on-surface-variant uppercase">{t("labFooterStat1Label")}</p>
            <p className="font-lab-body mt-1 text-sm font-semibold text-lab-on-surface">{t("labFooterStat1Value")}</p>
          </div>
          <div className="rounded-xl border border-lab-outline-variant bg-lab-surface px-4 py-4 shadow-sm backdrop-blur-sm" data-footer-showcase>
            <p className="font-lab-mono text-[10px] font-semibold tracking-wider text-lab-on-surface-variant uppercase">{t("labFooterStat2Label")}</p>
            <p className="font-lab-body mt-1 text-sm font-semibold text-lab-on-surface">{t("labFooterStat2Value")}</p>
          </div>
          <div className="rounded-xl border border-lab-outline-variant bg-lab-surface px-4 py-4 shadow-sm backdrop-blur-sm" data-footer-showcase>
            <p className="font-lab-mono text-[10px] font-semibold tracking-wider text-lab-on-surface-variant uppercase">{t("labFooterStat3Label")}</p>
            <p className="font-lab-body mt-1 text-sm font-semibold text-lab-on-surface">{t("labFooterStat3Value")}</p>
          </div>
        </div>

        <div
          className="relative mx-auto mt-8 flex max-w-6xl flex-col items-center justify-between gap-5 border-t border-lab-outline-variant pt-6 md:flex-row"
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 md:justify-end">
            <Link
              href="/methodology"
              className="font-lab-mono rounded-sm border border-lab-outline-variant bg-lab-surface px-3 py-1.5 text-[10px] font-semibold tracking-wide text-lab-on-surface uppercase transition-all hover:border-lab-on-surface/30"
            >
              {t("labFooterMethodology")}
            </Link>
            <span className="font-lab-mono rounded-sm border border-lab-outline-variant bg-lab-surface px-3 py-1.5 text-[10px] font-semibold tracking-wide text-lab-on-surface uppercase">
              {t("labFooterPrivacy")}
            </span>
            <span className="font-lab-mono rounded-sm border border-lab-outline-variant bg-lab-surface px-3 py-1.5 text-[10px] font-semibold tracking-wide text-lab-on-surface uppercase">
              {t("labFooterSocials")}
