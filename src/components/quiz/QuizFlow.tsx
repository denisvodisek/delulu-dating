...
const choiceActive =
  "bg-lab-primary text-lab-on-primary ring-2 ring-lab-on-surface/15 shadow-sm";

const choiceInactive = "bg-lab-surface-container-lowest text-lab-on-surface-variant hover:bg-lab-surface";

const choiceInactiveMuted =
  "bg-lab-surface-container-lowest text-lab-on-surface-variant hover:bg-lab-surface";
    ...
    <div className="grid h-8 grid-cols-10 gap-1">
      {Array.from({ length: 10 }, (_, i) => (
        <div
          ...
          className={cn(
            "rounded-md",
            i < safe ? "bg-lab-primary shadow-sm" : "bg-lab-outline-variant",
          )}
        />
      ))}
    </div>
...
function SegmentedTwo({
  ...
  return (
    <div className="grid grid-cols-2 gap-0 overflow-hidden rounded-xl border border-lab-outline-variant shadow-inner">
      <button
        type="button"
        onClick={onPickA}
        className={cn(
          "font-lab-mono text-lab-mono border-r border-lab-outline-variant py-4 text-xs font-semibold uppercase tracking-wide transition-all duration-200",
...
export default function QuizFlow() {
  ...
  return (
    <div className="quiz-flow-root relative min-h-[calc(100dvh-5rem)] pb-36">
      <div
        className="fixed top-20 right-0 left-0 z-40 h-1.5 w-full bg-lab-outline-variant"
        aria-hidden
      >
        <div
          className="h-full bg-lab-primary transition-[width] duration-300 ease-out"
          style={{ width: `${progressPct}%` }}
        />
      </div>

      <main className="flex min-h-[calc(100dvh-5rem-6px)] items-start justify-center px-4 pt-28 pb-28 md:px-16">
        <div className="grid w-full max-w-6xl grid-cols-1 overflow-hidden rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest/95 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.12)] backdrop-blur-xl md:grid-cols-12">
          {/* Left: question + roast + stats */}
          <div className="flex flex-col justify-center gap-8 border-b border-lab-outline-variant bg-lab-surface p-8 md:col-span-5 md:border-r md:border-b-0 md:p-12">
            <div className="space-y-2">
              <span className="font-lab-mono text-lab-mono text-xs font-semibold tracking-[0.14em] text-lab-on-surface-variant uppercase">
                {t("labQuestionKicker")}{" "}
            ...
            <div className="rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest p-6 shadow-sm">
              <div className="flex gap-3">
                <span className="material-symbols-outlined shrink-0 text-lab-on-surface">monitoring</span>
          ...
          {/* Right: controls */}
          <div className="flex flex-col justify-center gap-10 bg-lab-surface-container-lowest p-8 md:col-span-7 md:p-12">
            {step === 0 && (
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <div className="flex flex-col overflow-hidden rounded-2xl border border-lab-outline-variant">
                  {(
                    [
            ...
            {step === 5 && (
              <div className="space-y-4">
                ...
                <div className="grid max-h-[min(22rem,calc(100dvh-15.5rem))] grid-cols-2 gap-0 overflow-y-auto overscroll-contain rounded-xl border border-lab-outline-variant sm:grid-cols-3">
                  {districtKeys.map((key) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "font-lab-mono text-lab-mono min-h-11 border-b border-r border-lab-outline-variant px-2 py-2.5 text-left text-[11px] font-semibold uppercase leading-tight transition-all duration-200 sm:text-xs",
                          on
                            ? "bg-lab-primary text-lab-on-primary shadow-inner ring-1 ring-lab-on-surface/10 ring-inset"
                            : "bg-lab-surface-container-lowest text-lab-on-surface-variant hover:bg-lab-surface",
                        )}
                      >
                        {td(key)}
            ...
            <p className="font-lab-mono text-center text-[10px] font-semibold tracking-widest text-lab-on-surface-variant uppercase">
              {t("noSpoilers")}
            </p>
          </div>
        </div>
      ...
      <footer className="fixed bottom-0 left-0 z-50 flex w-full flex-col items-center gap-4 border-t border-lab-outline-variant bg-lab-surface-container-lowest/95 px-4 py-6 shadow-[0_-4px_24px_rgba(0,0,0,0.06)] backdrop-blur-xl md:flex-row md:justify-between md:px-16">
        <div className="flex flex-wrap items-center justify-center gap-6 md:justify-start">
          <button
            type="button"
            onClick={back}
            disabled={step === 0 || isRevealing}
            className="font-lab-mono flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-lab-on-surface-variant transition-colors hover:text-lab-on-surface disabled:opacity-40"
          >
          ...
          <button
            ...
            className="font-lab-mono text-[10px] font-semibold uppercase tracking-wide text-lab-on-surface-variant underline-offset-4 transition-colors hover:text-lab-on-surface hover:underline disabled:opacity-40"
          >
            {t("skip")}
          </button>
        ...
        <button
          ...
          onClick={next}
          disabled={isRevealing}
          className="font-lab-mono text-lab-mono flex w-full items-center justify-center gap-2 border-2 border-lab-on-surface bg-lab-primary py-4 text-xs font-bold uppercase tracking-wide text-lab-on-primary shadow-[3px_3px_0_0_rgb(10,10,11)] transition-all hover:translate-x-px hover:translate-y-px hover:shadow-[2px_2px_0_0_rgb(10,10,11)] disabled:opacity-50 md:w-auto md:px-12"
        >
      ...
      {isRevealing ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-lab-ink/40 px-6 backdrop-blur-md">
          <div className="relative w-full max-w-md overflow-hidden rounded-xl border border-lab-outline-variant bg-lab-surface-container-lowest p-8 shadow-[0_24px_80px_rgba(0,0,0,0.18)] sm:max-w-lg">
            <div className="pointer-events-none absolute -top-16 -left-10 h-32 w-32 rounded-full bg-lab-primary/20 blur-2xl" />
            <div className="pointer-events-none absolute -right-12 -bottom-14 h-40 w-40 rounded-full bg-lab-tertiary/15 blur-2xl" />
            <div className="font-lab-mono relative mb-4 flex items-center gap-2 text-xs font-semibold tracking-wide text-lab-on-surface uppercase">
              <CircleNotch className="text-lab-tertiary animate-spin" size={18} />
              {t("labRevealTitle")}
            </div>
            <div className="relative mb-4 h-2 overflow-hidden rounded-sm bg-lab-outline-variant">
              <div
                className="h-full rounded-sm bg-lab-primary transition-[width] duration-200"
            ...
            <div className="relative mt-5 flex items-center gap-2">
              <span className="h-2 w-2 animate-bounce rounded-full bg-lab-primary [animation-delay:-0.25s]" />
              <span className="bg-lab-tertiary h-2 w-2 animate-bounce rounded-full [animation-delay:-0.12s]" />
              <span className="h-2 w-2 animate-bounce rounded-full bg-lab-outline" />
            </div>
          </div>
        </div>
      ) : null}
